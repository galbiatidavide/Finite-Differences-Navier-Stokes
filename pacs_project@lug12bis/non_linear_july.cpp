...
static PetscScalar solution(PetscScalar x, PetscScalar y, PetscScalar z)
{
    ...
    //return -4*pi*cos(2*pi*x)*cos(4*pi*y)*cos(4*pi*y)*cos(8*pi*z)*cos(8*pi*z)*sin(2*pi*x) - 8*pi*cos(2*pi*x)*cos(2*pi*x)*cos(4*pi*y)*cos(8*pi*z)*cos(8*pi*z)*sin(4*pi*y);
    //return x;
}
...
static PetscErrorCode ManageAdvection(PetscScalar dt, Vec* U_int, Vec* V_int, Vec* W_int, Vec U_n, Vec V_n, Vec W_n, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    Vec my_bench;
    DMCreateGlobalVector(dmSol_Staggered, &my_bench);
    CreateReferenceSolutionTry(dmSol_Staggered, &my_bench);

    //////////////////////////////////

        CheckSolution(homoThird, my_bench); 

        PetscViewer viewer_P;
        DM da_solution_P;
        DMStagCreateCompatibleDMStag(dmSol_Staggered, 0, 0, 1, 0, &da_solution_P); 
        Vec P_grid;
        DMStagVecSplitToDMDA(dmSol_Staggered, homoThird, BACK, 0, &da_solution_P, &P_grid);
