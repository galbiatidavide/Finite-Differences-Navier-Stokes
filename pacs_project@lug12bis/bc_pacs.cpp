...
{
    return sin(pi*x)*cos(2*pi*y) + pi*sin(4*pi*y);
    //return 0;
    //(2*pi*sin(pi*x)+sin(2*pi*y));
    ...
    for (ez = startz; ez < startz + nz; ++ez) { /* With DMStag, always iterate x fastest, y second fastest, z slowest */
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                row.loc = LEFT;
                row.c = 0;
                inter = solution(arrCoord[ez][ey][ex][icuy[0]], arrCoord[ez][ey][ex][icuy[1]],
...
static PetscErrorCode ManageAdvection(PetscScalar dt, Vec* U_int, Vec* V_int, Vec* W_int, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    PetscViewer viewer_U;
            ...
            DMStagVecSplitToDMDA(dmSol_Staggered, V_n, DOWN, 0, &da_solution_U, &U_grid);
       
      
        
            PetscObjectSetName((PetscObject)U_grid, "U_my");
     

            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_U), "U_my.vtr", FILE_MODE_WRITE, &viewer_U);
...
static PetscErrorCode Assemble_x(DM dmSol, Mat *pA, Vec *pRhs, Vec solRef, PetscScalar dt, PetscScalar Re) {
    ...
    Mat A;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux[3], icuy[3], icuz[3], icux_right[3], icuy_up[3], icuz_front[3], icux_up_left[3], icux_front_left[3], icux_back_left[3], icux_down_left[3];
    PetscReal hx, hy, hz;
...
static PetscErrorCode Assemble_y(DM dmSol, Mat *pA, Vec *pRhs, Vec solRef, PetscScalar dt, PetscScalar Re) {
    ...
    Mat A;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux[3], icuy[3], icuz[3], icux_right[3], icuy_up[3], icuz_front[3], icuy_back_down[3], icuy_down_left[3], icuy_down_right[3], icuy_front_down[3];
    PetscReal hx, hy, hz;
...
static PetscErrorCode Assemble_z(DM dmSol, Mat *pA, Vec *pRhs, Vec solRef, PetscScalar dt, PetscScalar Re) {
    ...
    Mat A;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux[3], icuy[3], icuz[3], icux_right[3], icuy_up[3], icuz_front[3], icuz_back_left[3], icuz_back_right[3], icuz_back_up[3], icuz_back_down[3];
    PetscReal hx, hy, hz;
...
static PetscErrorCode AssembleDivergence(DM dmSol, Vec *pDiv, Vec U_x, Vec U_y, Vec U_z) 
{
    ...
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux[3], icux_right[3], icuy_down[3], icuy_up[3], icuz_back[3], icuz_front[3];
    DM dmCoord;
    PetscScalar ****arrCoord, ****array;
    PetscReal hx, hy, hz;
    ...
    Vec l_y;
    DMCreateLocalVector(dmSol,&l_y);
    DMGlobalToLocalBegin(dmSol,U_y,INSERT_VALUES,l_y);
    
    Vec l_z;
    DMCreateLocalVector(dmSol,&l_z);
    DMGlobalToLocalBegin(dmSol,U_z,INSERT_VALUES,l_z);
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                row_back.c = 0;
                    ...
                    DMStagVecGetValuesStencil(dmSol, l_y, 1, &row_up, &up);
                DMStagVecGetValuesStencil(dmSol, l_y, 1, &row_down, &down);
                DMStagVecGetValuesStencil(dmSol, l_x, 1, &row_left, &left);
                DMStagVecGetValuesStencil(dmSol, l_x, 1, &row_right, &right);
                DMStagVecGetValuesStencil(dmSol, l_z, 1, &row_front, &front);
                DMStagVecGetValuesStencil(dmSol, l_z, 1, &row_back, &back);
...
static PetscErrorCode ComputeDivergence(Vec* pDiv, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz) 
{
...
static PetscErrorCode ManagePressure(PetscScalar dt, Vec* pP, Vec* pP_x, Vec*  pP_y, Vec* pP_z, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    Derive_x(dmSol_shifted, &P_x, P_shifted);
    Derive_y(dmSol_shifted, &P_y, P_shifted);
    Derive_z(dmSol_shifted, &P_z, P_shifted);
...
int main(int argc, char **argv)
{
    ...
    // Create necessary grids
    DM dmSol_Shifted, dmSol_Centered, dmSol_Staggered, dmSol_centered;
    {
        CreateGrid(&dmSol_Shifted, 1, 1, 0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
        CreateGrid(&dmSol_Centered, 0, 0, 1, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
        CreateGrid(&dmSol_Staggered, 0, 1, 0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
    ...
    CreateReferenceSolution(dmSol_Staggered, &sol);
...
*/
   ...
   // {
        //if(i == 1)
        {
            ...
            /* Managing second step: find the preliminary velocity field*/
            ManageViscosity(dt, Re, &U_pre, &V_pre, &W_pre, U_int, V_int, W_int, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);

            /* Managing third step: find pressure field and its derivartives*/
...
///////////////////////
            ...
            DMStagVecSplitToDMDA(dmSol_Staggered, U_pre, LEFT, 0, &da_solution_U, &U_grid);
            DMStagVecSplitToDMDA(dmSol_Staggered, V_pre, DOWN, 0, &da_solution_V, &V_grid); 
            DMStagVecSplitToDMDA(dmSol_Staggered, W_pre, BACK, 0, &da_solution_W, &W_grid); 
            DMStagVecSplitToDMDA(dmSol_Staggered, P, ELEMENT, 0, &da_solution_P, &P_grid); 

            int p;
            double val;
            VecMax(U_int, &p, &val);
            std::cout<<val;
            //CheckSolution(Vec sol, Vec solRef)
            ...
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_V), "V.vtr", FILE_MODE_WRITE, &viewer_V);
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_W), "W.vtr", FILE_MODE_WRITE, &viewer_W);
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "W.vtr", FILE_MODE_WRITE, &viewer_P);
            ...
            //DumpToVTK(dmSol_Staggered, dmSol_Centered, U_up, V_up, W_up, P);
