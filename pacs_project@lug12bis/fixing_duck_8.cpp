...
{
    return  cos(pi*x)*sin(2*pi*y)*cos(2*pi*z);
    //return  0*x + 0*y;
...
{
    return  -sin(pi*x)*cos(2*pi*y)*cos(2*pi*z);
    //return  0*x + 0*y;
}

static PetscScalar uzRef(PetscScalar x, PetscScalar y, PetscScalar z)
{
    return  cos(pi*x)*cos(2*pi*y)*cos(2*pi*z);
}
...
static PetscScalar solution(PetscScalar x, PetscScalar y, PetscScalar z)
{
    ...
    return  9*0.02*pi*pi*cos(pi*x)*cos(2*pi*z)*sin(2*pi*y);
}
...
int main(int argc, char **argv)
{
    ...
    PetscObjectSetName((PetscObject)P_grid, "pre_my");
    PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "pre_my.vtr", FILE_MODE_WRITE, &viewer_P);            
