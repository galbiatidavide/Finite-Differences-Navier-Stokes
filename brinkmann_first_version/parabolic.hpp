...
private:
    ...
    const PetscErrorCode assemble_rhs(PetscReal const & theta)
    {
        ...
        DMGlobalToLocalBegin(dmGrid, penalization_u, INSERT_VALUES, penalizationLocal);
        DMGlobalToLocalEnd(dmGrid, penalization_u, INSERT_VALUES, penalizationLocal);
...
protected:
    ...
    const PetscErrorCode assemble_lhs() 
    {
        ...
        DMGlobalToLocalBegin(dmGrid, penalization_u, INSERT_VALUES, penalization_local);
        DMGlobalToLocalEnd(dmGrid, penalization_u, INSERT_VALUES, penalization_local);
...
public:
    ...
    parabolic_problem_x(Vec const & U_0, Vec const & penalization_u, DM const & dmGrid, PetscReal const & Re, PetscReal const & dt, PetscReal const & iter):
    Re(Re), dt(dt), iter(iter), dmGrid(dmGrid), penalization_u(penalization_u)
    {   
    ...
    Vec get_U() const { return U_up; }

    void set_U(Vec U) { VecCopy(U, U_up); }

    virtual ~parabolic_problem_x()
    {
        VecDestroy(&U_up);
        MatDestroy(&A);
        VecDestroy(&rhs);
...
private:
    ...
    const PetscErrorCode assemble_rhs(PetscReal const & theta)
    {
        ...
        PetscReal ****arrPenalization;
        DMCreateLocalVector(dmGrid, &penalizationLocal);
        DMGlobalToLocalBegin(dmGrid, penalization_v, INSERT_VALUES, penalizationLocal);
        DMGlobalToLocalEnd(dmGrid, penalization_v, INSERT_VALUES, penalizationLocal);
        DMStagVecGetArrayRead(dmGrid, penalizationLocal, &arrPenalization); 
...
protected:
    ...
    const PetscErrorCode assemble_lhs()
    {
        ...
        PetscReal ****arrPenalization;
        DMCreateLocalVector(dmGrid, &penalization_local);
        DMGlobalToLocalBegin(dmGrid, penalization_v, INSERT_VALUES, penalization_local);
        DMGlobalToLocalEnd(dmGrid, penalization_v, INSERT_VALUES, penalization_local);
        DMStagVecGetArrayRead(dmGrid, penalization_local, &arrPenalization);
...
public:
    ...
    parabolic_problem_y(Vec const & V_0, Vec const & penalization_v, DM const & dmGrid, PetscReal const & Re, PetscReal const & dt, PetscReal const & iter):
    Re(Re), dt(dt), iter(iter), dmGrid(dmGrid), penalization_v(penalization_v)
    {
        DMCreateMatrix(dmGrid, &A);
        DMCreateGlobalVector(dmGrid, &rhs);
...
private:
    ...
    PetscErrorCode const assemble_rhs(PetscReal const & theta) 
    {
        ...
        PetscReal ****arrPenalization;
        DMCreateLocalVector(dmGrid, &penalizationLocal);
        DMGlobalToLocalBegin(dmGrid, penalization_w, INSERT_VALUES, penalizationLocal);
        DMGlobalToLocalEnd(dmGrid, penalization_w, INSERT_VALUES, penalizationLocal);
        DMStagVecGetArrayRead(dmGrid, penalizationLocal, &arrPenalization); 
...
protected:
    ...
    PetscErrorCode const assemble_lhs() 
    {
        ...
        PetscReal ****arrPenalization;
        DMCreateLocalVector(dmGrid, &penalization_local);
        DMGlobalToLocalBegin(dmGrid, penalization_w, INSERT_VALUES, penalization_local);
        DMGlobalToLocalEnd(dmGrid, penalization_w, INSERT_VALUES, penalization_local);
        DMStagVecGetArrayRead(dmGrid, penalization_local, &arrPenalization);
...
public:
    ...
    parabolic_problem_z(Vec const & W_0, Vec const & penalization_w, DM const & dmGrid, PetscReal const & Re, PetscReal const & dt, PetscReal const & iter):
    Re(Re), dt(dt), iter(iter), dmGrid(dmGrid), penalization_w(penalization_w)
    {
        DMCreateMatrix(dmGrid, &A);
        DMCreateGlobalVector(dmGrid, &rhs);
    ...
    Vec get_W() const { return W_up; }

    void set_W(Vec W) { VecCopy(W, W_up); }
    
    virtual ~parabolic_problem_z()
    {
        VecDestroy(&W_up);
        MatDestroy(&A);
        VecDestroy(&rhs);
