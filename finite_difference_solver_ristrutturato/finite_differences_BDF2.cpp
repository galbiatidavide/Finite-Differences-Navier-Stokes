                ...
                if (ex == 0) {
                    ...
                    if (ey == 0) {
                        if (ez == 0) {
                            ...
                            valRhs = -Ret2*valRhs - bc_1/(hy*hy) - bc_2/(hz*hz);
                            DMStagVecSetValuesStencil(dmGrid, rhs, 1, &row, &valRhs, INSERT_VALUES);                              
                        } else if (ez == N[2] - 1) {
                            nEntries = 5;
                            col[0].i = ex;
                ...
                if (ey == 0) {
                    ...
                    if (ex == 0) {
                        if (ez == 0) {
                            ...
                            valRhs = -Ret2*valRhs - bc_1/(hx*hx) - bc_2/(hz*hz);
                            DMStagVecSetValuesStencil(dmGrid, rhs, 1, &row, &valRhs, INSERT_VALUES);
                        } else if (ez == N[2] - 1) {
                            nEntries   = 5;
                            col[0].i   = ex;
                ...
                if (ez == 0) {
                    ...
                    if (ex == 0) {
                        if (ey == 0) {
                            ...
                            valRhs = -Ret2*valRhs - bc_2/(hy*hy) - bc_1/(hx*hx);                            
                            DMStagVecSetValuesStencil(dmGrid, rhs, 1, &row, &valRhs, INSERT_VALUES); 
                        } else if (ey == N[1] - 1) {
                            nEntries   = 5;
                            col[0].i   = ex;
    ...
    for (ez = startz; ez < startz + nz; ++ez) { 
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                inter = 1.5*((up - down) / hy + (right - left) / hx + (front - back) / hz)/dt;
...
PetscErrorCode UpdateVelocity(DM const & dmGrid_staggered_x, DM const & dmGrid_staggered_y, DM const & dmGrid_staggered_z, PetscReal const & dt, Vec & U_up, Vec & V_up, Vec & W_up, Vec const & P_x, Vec const & P_y, Vec const & P_z, Vec const & U_pre, Vec const & V_pre, Vec const & W_pre, PetscReal const & theta)
{
    ...
    VecAXPY(U_pre, -(2.0/3.0)*dt, P_x);
    VecAXPY(V_pre, -(2.0/3.0)*dt, P_y);
    VecAXPY(W_pre, -(2.0/3.0)*dt, P_z);
...
{   
    ...
    DMCreateGlobalVector(dmGrid_Staggered_z, &P_z);
            ...
            Vec w;
            DMStagVecSplitToDMDA(dmGrid_Staggered_y, P_y_pre, DOWN, 0, &DM_w, &w);
            PetscObjectSetName((PetscObject)w, "cristo");
            char filename_w[50];
            sprintf(filename_w, "results/cristo.vtr");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)dmGrid_Staggered_y), filename_w, FILE_MODE_WRITE, &viewer_w);
            VecView(w, viewer_w);
            VecDestroy(&w);
            DMDestroy(&DM_w);
    ...
    for(size_t i = 0; i < iter; ++i){
        if (i == 0){
            ...
            ManageAdvection_x(dt, U_int, U_0, V_0, W_0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, U_prev);
            ManageAdvection_y(dt, V_int, U_0, V_0, W_0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, V_prev);
            ManageAdvection_z(dt, W_int, U_0, V_0, W_0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, W_prev);
            ...
            ManageAdvection_x(dt, U_int, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, U_prev);
            ManageAdvection_y(dt, V_int, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, V_prev);
            ManageAdvection_z(dt, W_int, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, W_prev); 
