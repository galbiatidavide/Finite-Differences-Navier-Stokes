...
int main(int argc, char **argv)
{
    ...
    // Create necessary grids
    DM dmGrid_Staggered_x, dmGrid_Staggered_y, dmGrid_Staggered_z; //need to declare to due to solving linear system laplacian messing things up
    {
    ...
    transport_problem_x transport(setting, U_0, V_0, W_0); 

    //transport.solve(); //non va

    //transport.solve_step_x(0);

    transport.solve_x();

            U_int = transport.get_U();
            Vec bench_x;
            DMCreateGlobalVector(dmGrid_Staggered_x, &bench_x);
            CreateReferenceSolutionTry(dmGrid_Staggered_x, &bench_x, 0);
            CheckSolution(U_int, bench_x);


    
    transport.~transport_problem_x();
