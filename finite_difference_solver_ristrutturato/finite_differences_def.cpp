...
// Assembling advection term
PetscErrorCode ManageAdvection_x_BDF2(PetscReal const & dt, Vec & U_int, Vec const & U_0, Vec const & V_0, Vec const & W_0, PetscInt const & nx, PetscInt const & ny, PetscInt const & nz, PetscReal const & Lx_0, PetscReal const & Lx, PetscReal const & Ly_0, PetscReal const & Ly, PetscReal const & Lz_0, PetscReal const & Lz, PetscReal const & theta, Vec const & U_prev)
{
    // Create necessary grids
    DM dmGrid_Shifted, dmGrid_Centered, dmGrid_Staggered;
    PetscFunctionBegin;
    ...
    VecAXPBYPCZ(U_n,-1.0,-2.0*dt,4.0,U_prev,homoFirst); 
...
PetscErrorCode ManageAdvection_y_BDF2(PetscReal const & dt, Vec & V_int, Vec const & U_0, Vec const & V_0, Vec const & W_0, PetscInt const & nx, PetscInt const & ny, PetscInt const & nz, PetscReal const & Lx_0, PetscReal const & Lx, PetscReal const & Ly_0, PetscReal const & Ly, PetscReal const & Lz_0, PetscReal const & Lz, PetscReal const & theta, Vec const & V_prev)
{
    // Create necessary grids
    DM dmGrid_Shifted, dmGrid_Centered, dmGrid_Staggered;
    PetscFunctionBegin;
    ...
    VecAXPBY(homoSecond,1.0,1.0,mixedSecond);
    VecAXPBYPCZ(V_n,-1.0,-2.0*dt,4.0,V_prev,homoSecond); 
...
PetscErrorCode ManageAdvection_z_BDF2(PetscReal const & dt, Vec & W_int, Vec const & U_0, Vec const & V_0, Vec const & W_0, PetscInt const & nx, PetscInt const & ny, PetscInt const & nz, PetscReal const & Lx_0, PetscReal const & Lx, PetscReal const & Ly_0, PetscReal const & Ly, PetscReal const & Lz_0, PetscReal const & Lz, PetscReal const & theta, Vec const & W_prev)
{
    // Create necessary grids
    DM dmGrid_Shifted, dmGrid_Centered, dmGrid_Staggered;
    PetscFunctionBegin;
    ...
    VecAXPBY(homoThird,1.0,1.0,mixedThird);
    VecAXPBYPCZ(W_n,-1.0,-2.0*dt,4.0,W_prev,homoThird); 
...
// Second step: viscosity members to solve implicit laplacian
PetscErrorCode Assemble_x_BDF2(DM const & dmGrid, Mat & A, Vec & rhs, Vec const & vec, PetscReal const & dt, PetscReal const & Re, PetscReal const & theta) {

    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux[3], icux_right[3];
    PetscReal const Ret = 1.5*Re/dt;
    Vec coordLocal;
    DM dmCoord;
    PetscReal ****arrCoord;
...
PetscErrorCode Assemble_y_BDF2(DM const & dmGrid, Mat & A, Vec & rhs, Vec const & solRef, PetscReal const & dt, PetscReal const & Re, PetscReal const & theta) {
...
PetscErrorCode Assemble_z_BDF2(DM const & dmGrid, Mat & A, Vec & rhs, Vec const & solRef, PetscReal const & dt, PetscReal const & Re, PetscReal const & theta) {
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icuz[3], icuz_front[3];
    PetscReal const Ret = 1.5*Re/dt;
    Vec coordLocal;
...
PetscErrorCode ManageViscosity(DM const & dmGrid_x, DM const & dmGrid_y, DM const & dmGrid_z, PetscReal const & dt, PetscReal const & Re, Vec & U_pre, Vec & V_pre, Vec & W_pre, Vec const & U_int, Vec const & V_int, Vec const & W_int, PetscReal const & theta, Vec const & P_x_pre, Vec const & P_y_pre, Vec const & P_z_pre){
    ...
    VecAXPY(U_int,Re,P_x_pre);
    VecAXPY(V_int,Re,P_y_pre);
    VecAXPY(W_int,Re,P_z_pre);
...
// Assembling Viscosity term
PetscErrorCode ManageViscosity_BDF2(DM const & dmGrid_x, DM const & dmGrid_y, DM const & dmGrid_z, PetscReal const & dt, PetscReal const & Re, Vec & U_pre, Vec & V_pre, Vec & W_pre, Vec const & U_int, Vec const & V_int, Vec const & W_int, PetscReal const & theta, Vec const & P_x_pre, Vec const & P_y_pre, Vec const & P_z_pre){
    ...
    VecScale(U_int, -0.5*Re/dt);
    VecScale(V_int, -0.5*Re/dt);
    VecScale(W_int, -0.5*Re/dt);
    ...
    {
    Assemble_x_BDF2(dmGrid_x, A_x, rhs_x, U_int, dt, Re, theta);
    ...
    {
    Assemble_y_BDF2(dmGrid_y, A_y, rhs_y, V_int, dt, Re, theta);
    ...
    {
    Assemble_z_BDF2(dmGrid_z, A_z, rhs_z, W_int, dt, Re, theta);
...
PetscErrorCode ComputeDivergence_BDF2(DM const & dmGrid_centered, DM const & dmGrid_shifted, DM const & dmGrid_staggered, Vec & div, Vec const & U_n, Vec const & V_n, Vec const & W_n, PetscReal const & dt) 
{
        PetscFunctionBegin;
...
PetscErrorCode ManagePressure(DM const & dmGrid_centered, DM const & dmGrid_shifted, DM const & dmGrid_staggered, PetscReal const & dt, Vec & P, Vec const & U_pre, Vec const & V_pre, Vec const & W_pre)
{
    ...
    Vec laplacian_pressure;
    DMCreateGlobalVector(dmGrid_centered, &laplacian_pressure);
    laplacian_element_explicit(dmGrid_centered, laplacian_pressure, P);
    VecAXPY(div,1.0,laplacian_pressure);
...
PetscErrorCode ManagePressure_BDF2(DM const & dmGrid_centered, DM const & dmGrid_shifted, DM const & dmGrid_staggered, PetscReal const & dt, Vec & P, Vec const & U_pre, Vec const & V_pre, Vec const & W_pre)
{
    Mat A;
    Vec rhs;
    KSP ksp;
    ...
    Vec div;
    DMCreateGlobalVector(dmGrid_centered, &div);
    ComputeDivergence_BDF2(dmGrid_centered, dmGrid_shifted, dmGrid_staggered, div, U_n, V_n, W_n, dt); 
...
PetscErrorCode UpdateVelocity(DM const & dmGrid_staggered_x, DM const & dmGrid_staggered_y, DM const & dmGrid_staggered_z, PetscReal const & dt, Vec & U_up, Vec & V_up, Vec & W_up, Vec const & P_x, Vec const & P_y, Vec const & P_z, Vec const & U_pre, Vec const & V_pre, Vec const & W_pre, PetscReal const & theta, Vec const & P_x_pre, Vec const & P_y_pre, Vec const & P_z_pre)
{
    ...
    VecAXPBYPCZ(U_pre,-dt,dt,1.0,P_x,P_x_pre);
    VecAXPBYPCZ(V_pre,-dt,dt,1.0,P_y,P_y_pre);
    VecAXPBYPCZ(W_pre,-dt,dt,1.0,P_z,P_z_pre);
...
PetscErrorCode UpdateVelocity_BDF2(DM const & dmGrid_staggered_x, DM const & dmGrid_staggered_y, DM const & dmGrid_staggered_z, PetscReal const & dt, Vec & U_up, Vec & V_up, Vec & W_up, Vec const & P_x, Vec const & P_y, Vec const & P_z, Vec const & U_pre, Vec const & V_pre, Vec const & W_pre, PetscReal const & theta, Vec const & P_x_pre, Vec const & P_y_pre, Vec const & P_z_pre)
{
    ...
    VecAXPBYPCZ(U_pre,-(2.0/3.0)*dt,(2.0/3.0)*dt,1.0,P_x,P_x_pre);
    VecAXPBYPCZ(V_pre,-(2.0/3.0)*dt,(2.0/3.0)*dt,1.0,P_y,P_y_pre);
    VecAXPBYPCZ(W_pre,-(2.0/3.0)*dt,(2.0/3.0)*dt,1.0,P_z,P_z_pre);
...
{   
    ...
    Vec P_x_pre, P_y_pre, P_z_pre, P;
    DMCreateGlobalVector(dmGrid_Centered, &P);
    CreateReferencePressure(dmGrid_Centered, P, 0);
    ...
    ManagePressure_x(dmGrid_Staggered_x, dmGrid_Centered, dmGrid_Shifted, P_x_pre, P);
    ManagePressure_y(dmGrid_Staggered_y, dmGrid_Centered, dmGrid_Shifted, P_y_pre, P);
    ManagePressure_z(dmGrid_Staggered_z, dmGrid_Centered, dmGrid_Shifted, P_z_pre, P);
    ...
    Vec U_prev, V_prev, W_prev;
    DMCreateGlobalVector(dmGrid_Staggered_x, &U_prev);
    DMCreateGlobalVector(dmGrid_Staggered_y, &V_prev);
    DMCreateGlobalVector(dmGrid_Staggered_z, &W_prev);
...
/*
    ...
    for(size_t i = 0; i < iter; ++i){
        if (i == 0){
            ...
            ManageViscosity(dmGrid_Staggered_x, dmGrid_Staggered_y, dmGrid_Staggered_z, dt, Re, U_pre, V_pre, W_pre, U_int, V_int, W_int, theta, P_x_pre, P_y_pre, P_z_pre);
            ...
            UpdateVelocity(dmGrid_Staggered_x, dmGrid_Staggered_y, dmGrid_Staggered_z, dt, U_up, V_up, W_up, P_x, P_y, P_z, U_pre, V_pre, W_pre, theta, P_x_pre, P_y_pre, P_z_pre);
            ...
            ManageAdvection_x_BDF2(dt, U_int, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, U_prev);
            ManageAdvection_y_BDF2(dt, V_int, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, V_prev);
            ManageAdvection_z_BDF2(dt, W_int, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta, W_prev); 
            ...
            //theta = d*d*(i+1)*dt;
            ManageViscosity_BDF2(dmGrid_Staggered_x, dmGrid_Staggered_y, dmGrid_Staggered_z, dt, Re, U_pre, V_pre, W_pre, U_int, V_int, W_int, theta, P_x_pre, P_y_pre, P_z_pre);

            //std::cout << "Spirit of Nebraska completed:   diffusion done." << std::endl;
            ManagePressure_BDF2(dmGrid_Centered, dmGrid_Shifted, dmGrid_Staggered, dt, P, U_pre, V_pre, W_pre);
            Vec pressure_bench;
            ...
            //theta = 3*(vRef*A/Re)*k*k*dt*(i+1);
            theta = d*d*(i+1)*dt;
            UpdateVelocity_BDF2(dmGrid_Staggered_x, dmGrid_Staggered_y, dmGrid_Staggered_z, dt, U_up, V_up, W_up, P_x, P_y, P_z, U_pre, V_pre, W_pre, theta, P_x_pre, P_y_pre, P_z_pre);
