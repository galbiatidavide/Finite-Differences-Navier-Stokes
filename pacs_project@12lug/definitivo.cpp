...
static PetscErrorCode ManageAdvection(PetscScalar dt, Vec* U_int, Vec* V_int, Vec* W_int, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    DMStagVecSplitToDMDA(dmSol_Staggered, homoSecond, DOWN, 0, &da_solution_P, &P_grid);



    PetscObjectSetName((PetscObject)P_grid, "t");


    PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "t.vtr", FILE_MODE_WRITE, &viewer_P);
...
static PetscErrorCode ManagePressure(PetscScalar dt, Vec* pP, Vec* pP_x, Vec*  pP_y, Vec* pP_z, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    Vec prova;
    CreateReferenceSolutionPressure(dmSol_centered, &prova);
    ...
    DMStagVecSplitToDMDA(dmSol_centered, *pP, ELEMENT, 0, &da_solution_P, &P_grid);



    PetscObjectSetName((PetscObject)P_grid, "test_dav");


    PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "test_dav.vtr", FILE_MODE_WRITE, &viewer_P);
...
int main(int argc, char **argv)
{
    ...
    DM dmSol_Shifted, dmSol_Centered, dmSol_Staggered, dmSol_Carotid;
    {
        ...
        CreateGrid(&dmSol_Centered, 0, 0, 1, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
        CreateGrid(&dmSol_Staggered, 0, 1, 0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
        CreateGrid(&dmSol_Carotid, 0, 1, 1, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
    ...
    CreateReferenceSolution(dmSol_Staggered, &bench);    
    ...
    /*char dataFile[] = "/home/dave/lifex/tests/pacs_project/carotid.vtk";


    PetscViewer v;
    PetscViewerCreate(PETSC_COMM_WORLD,&v);
    PetscViewerSetType(v,PETSCVIEWERBINARY);
    PetscViewerBinaryOpen(PETSC_COMM_WORLD, dataFile, FILE_MODE_READ, &v);


    // Read the data
    PetscInt rows, cols;
    Mat C;
    DMCreateMatrix(dmSol_Carotid, &C);

    PetscViewerBinaryRead(v, &C ,1, NULL, PETSC_DOUBLE);

    // Close the PetscViewer
