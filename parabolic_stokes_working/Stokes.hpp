...
parabolic_problem_x pb_x;
parabolic_problem_y pb_y;
parabolic_problem_z pb_z;
...
PetscErrorCode const AssembleDivergence(Vec & div, Vec const & U, Vec const &  V, Vec const & W) 
{
...
PetscErrorCode const ComputeDivergence(Vec & div, Vec const & U_n, Vec const & V_n, Vec const & W_n) 
{
        PetscFunctionBegin;
        ...
        AssembleDivergence(div_shifted, U_shifted, V_shifted, W_shifted);
...
PetscErrorCode const ManagePressure_x()
{
    PetscFunctionBegin;
...
PetscErrorCode const ManagePressure_y()
{
    PetscFunctionBegin
...
PetscErrorCode const ManagePressure_z()
{
    PetscFunctionBegin
...
PetscErrorCode const ManagePressure()
{
    KSP ksp;
    PC  pc;
    ...
    ComputeDivergence(div, U_up, V_up, W_up);     
...
PetscErrorCode const UpdatebcU(PetscReal const & theta)
{
...
PetscErrorCode const UpdatebcV(PetscReal const & theta) 
{
...
PetscErrorCode const UpdatebcW(PetscReal const & theta) 
{
...
PetscErrorCode const UpdateVelocity(PetscReal const & theta)
{
    PetscFunctionBegin;
...
PetscErrorCode const AssembleMagnitude(Vec & Magnitude_Shifted, Vec const & U, Vec const & V, Vec const & W) 
{
    PetscInt iu_left, iu_right, iu_up, iu_down, iu_front, iu_back, iu_element;
...
PetscErrorCode const ComputeMagnitude()
{    
    PetscFunctionBegin;
    Vec U_shifted;
    DMCreateGlobalVector(dmGrid_Shifted, &U_shifted);
    ...
    AssembleMagnitude(magnitude_shifted, U_shifted, V_shifted, W_shifted);
...
stokes_problem(Vec const & U_0_, Vec const & V_0_, Vec const & W_0_, DM const & dmGrid_Staggered_x, DM const & dmGrid_Staggered_y, DM const & dmGrid_Staggered_z, DM const & dmGrid_Centered, DM const & dmGrid_Shifted, PetscReal const & dt, PetscInt const & iter, PetscReal const & Re) :
pb_x(U_0_, dmGrid_Staggered_x, Re, dt, iter),
pb_y(V_0_, dmGrid_Staggered_y, Re, dt, iter),
pb_z(W_0_, dmGrid_Staggered_z, Re, dt, iter),
...
PetscErrorCode const solve()
{
    ...
    /*ProblemSetting<Parabolic> setting_x(nx, ny, nz, Lx_0, Ly_0, Lz_0, Lx, Ly, Lz, dt, iter, 1.0/Re);
    ProblemSetting<Parabolic> setting_y(nx, ny, nz, Lx_0, Ly_0, Lz_0, Lx, Ly, Lz, dt, iter, 1.0/Re);
    ProblemSetting<Parabolic> setting_z(nx, ny, nz, Lx_0, Ly_0, Lz_0, Lx, Ly, Lz, dt, iter, 1.0/Re);*/
    pb_x.assemble_lhs();
    pb_y.assemble_lhs();
    pb_z.assemble_lhs();
    ...
    for(size_t i = 0; i < iter; ++i){
        ...
        pb_x.solve_step(theta);
        pb_y.solve_step(theta);
        pb_z.solve_step(theta);
        U_up = pb_x.get_U();
        V_up = pb_y.get_V();
        W_up = pb_z.get_W();
    ...
    setting_x.~ProblemSetting();
    setting_y.~ProblemSetting();
    setting_z.~ProblemSetting();*/

    PetscFunctionReturn(0);
}

virtual ~stokes_problem()
{
    VecDestroy(&U_up);
    VecDestroy(&V_up);
    VecDestroy(&W_up);
