...

{
    ...
    return A*(sin(k*x - c)*cos(k*y - d)*sin(k*z) - cos(k*z - c)*sin(k*x - d)*sin(k*y))*exp(-theta);
}
...
static PetscScalar uyRef(PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar theta)
{
    ...
    return A*(sin(k*y - c)*cos(k*z - d)*sin(k*x) - cos(k*x - c)*sin(k*y - d)*sin(k*z))*exp(-theta);
    //return cos(2*pi*x)*cos(4*pi*y)*cos(8*pi*z);
...
{
    //return  -a*(exp(a*z)*sin(a*x+d*y)+exp(a*y)*cos(a*z+d*x))*exp(-theta)*par;
    return A*(sin(k*z - c)*cos(k*x - d)*sin(k*y) - cos(k*y - c)*sin(k*z - d)*sin(k*x))*exp(-theta);
    //return cos(2*pi*x)*cos(4*pi*y)*cos(8*pi*z);
...
static PetscErrorCode CreateReferenceSolutionFirst(DM dmSol, Vec *pSolRef, PetscScalar theta)
{
...
static PetscErrorCode CreateReferenceSolutionSecond(DM dmSol, Vec *pSolRef, PetscScalar theta)
{
    PetscInt        start[3], n[3], nExtra[3], ex, ey, ez, d;
    PetscInt        iux, iuy, iuz, icux[3], icuy[3], icuz[3];
    Vec             solRef, solRefLocal, coord, coordLocal;
...
static PetscErrorCode CreateReferenceSolutionThird(DM dmSol, Vec *pSolRef, PetscScalar theta)
{
    PetscInt        start[3], n[3], nExtra[3], ex, ey, ez, d;
    PetscInt        iux, iuy, iuz, icux[3], icuy[3], icuz[3];
    Vec             solRef, solRefLocal, coord, coordLocal;
...
static PetscErrorCode CreateReferenceSolutionTry(DM dmSol, Vec *pSolRef, PetscScalar theta)
{
    PetscInt        start[3], n[3], nExtra[3], ex, ey, ez, d;
    PetscInt        iux, iuy, iuz, icux[3], icuy[3], icuz[3];
    Vec             solRef, solRefLocal, coord, coordLocal;
...
static PetscErrorCode FirstShiftU_y(DM dmSol, Vec *pUShifted, Vec solRef, PetscScalar theta) //ok
{
...
static PetscErrorCode FirstShiftU_z(DM dmSol, Vec *pUShifted, Vec solRef, PetscScalar theta) //ok
{
...
static PetscErrorCode FirstShiftV_y(DM dmSol, Vec *pVShifted, Vec solRef, PetscScalar theta) //ok
{
...
static PetscErrorCode FirstShiftW_z(DM dmSol, Vec *pWShifted, Vec solRef, PetscScalar theta) //ok
{
...
static PetscErrorCode SecondShiftV_z(DM dmSol, Vec *pVShifted, Vec solRef, PetscScalar theta) //ok
{
...
static PetscErrorCode SecondShiftW_z(DM dmSol, Vec *pWShifted, Vec solRef, PetscScalar theta) //ok
{
...
// non-linear members: homogeneous
static PetscErrorCode CenterU(DM dmSol, Vec *pUCenter, Vec solRef, PetscScalar theta)
{
            ...
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ex == N[0] - 1) {
                    ...
                    inter = ((prev + uxRef(arrCoord[ez][ey][ex][icux_right[0]], arrCoord[ez][ey][ex][icux_right[1]],
                                           arrCoord[ez][ey][ex][icux_right[2]], theta)) / 2.0) * ((prev + uxRef(arrCoord[ez][ey][ex][icux_right[0]], arrCoord[ez][ey][ex][icux_right[1]],
                                                                                                         arrCoord[ez][ey][ex][icux_right[2]], theta)) / 2.0);
                    ...
                    inter = ((prev + uxRef(arrCoord[ez][ey][ex][icux[0]], arrCoord[ez][ey][ex][icux[1]],
                                           arrCoord[ez][ey][ex][icux[2]], theta)) / 2.0) * ((prev + uxRef(arrCoord[ez][ey][ex][icux[0]], arrCoord[ez][ey][ex][icux[1]],
                                                                                                   arrCoord[ez][ey][ex][icux[2]], theta)) / 2.0);
                    DMStagVecSetValuesStencil(dmSol, UCenter, 1, &row, &inter, INSERT_VALUES);
...
static PetscErrorCode CenterV(DM dmSol, Vec *pVCenter, Vec solRef, PetscScalar theta) 
{
            ...
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ey == N[1] - 1) {
                    ...
                    inter = ((prev + uyRef(arrCoord[ez][ey][ex][icuy_up[0]], arrCoord[ez][ey][ex][icuy_up[1]],
                                           arrCoord[ez][ey][ex][icuy_up[2]], theta)) / 2.0) * ((prev + uyRef(arrCoord[ez][ey][ex][icuy_up[0]], arrCoord[ez][ey][ex][icuy_up[1]],
                                                                                                      arrCoord[ez][ey][ex][icuy_up[2]], theta)) / 2.0);
                    ...
                    inter = ((prev + uyRef(arrCoord[ez][ey][ex][icuy[0]], arrCoord[ez][ey][ex][icuy[1]],
                                           arrCoord[ez][ey][ex][icuy[2]], theta)) / 2.0) * ((prev + uyRef(arrCoord[ez][ey][ex][icuy[0]], arrCoord[ez][ey][ex][icuy[1]],
                                                                                                   arrCoord[ez][ey][ex][icuy[2]], theta)) / 2.0);
                    DMStagVecSetValuesStencil(dmSol, VCenter, 1, &row, &inter, INSERT_VALUES);
...
static PetscErrorCode CenterW(DM dmSol, Vec *pWCenter, Vec solRef, PetscScalar theta) 
{
            ...
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ez == N[2] - 1) {
                    ...
                    inter = ((prev + uzRef(arrCoord[ez][ey][ex][icuz_front[0]], arrCoord[ez][ey][ex][icuz_front[1]],
                                           arrCoord[ez][ey][ex][icuz_front[2]], theta)) / 2.0) * ((prev + uzRef(arrCoord[ez][ey][ex][icuz_front[0]], arrCoord[ez][ey][ex][icuz_front[1]],
                                                                                                         arrCoord[ez][ey][ex][icuz_front[2]], theta)) / 2.0);
                    ...
                    inter = ((prev + uzRef(arrCoord[ez][ey][ex][icuz[0]], arrCoord[ez][ey][ex][icuz[1]],
                                           arrCoord[ez][ey][ex][icuz[2]], theta)) / 2.0) * ((prev + uzRef(arrCoord[ez][ey][ex][icuz[0]], arrCoord[ez][ey][ex][icuz[1]],
                                                                                                   arrCoord[ez][ey][ex][icuz[2]], theta)) / 2.0);
                    DMStagVecSetValuesStencil(dmSol, WCenter, 1, &row, &inter, INSERT_VALUES);
...
static PetscErrorCode CenterU_magnitude(DM dmSol, Vec *pUCenter, Vec solRef, PetscScalar theta)
{
...
static PetscErrorCode CenterV_magnitude(DM dmSol, Vec *pVCenter, Vec solRef, PetscScalar theta) 
{
...
static PetscErrorCode CenterW_magnitude(DM dmSol, Vec *pWCenter, Vec solRef, PetscScalar theta) 
{
...
static PetscErrorCode Derive_x(DM dmSol, Vec *pU2_x, Vec U2, PetscScalar theta)
{
    Vec             U2_x, coordLocal;
    PetscInt        startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscReal       hx;
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if(ex == 0) {
                        ...
                        val_l = (val_left - uxRef(arrCoord[ez][ey][ex][icux_e[0]] - hx, arrCoord[ez][ey][ex][icux_e[1]], arrCoord[ez][ey][ex][icux_e[2]], theta)*uxRef(arrCoord[ez][ey][ex][icux_e[0]] - hx, arrCoord[ez][ey][ex][icux_e[1]],
                                            arrCoord[ez][ey][ex][icux_e[2]], theta))/hx;
                        DMStagVecSetValuesStencil(dmSol, U2_x, 1, &row_l, &val_l, INSERT_VALUES);
                ...
                if(ex == N[0] - 1){
                        ...
                        val_r = (uxRef(arrCoord[ez][ey][ex][icux_e[0]] + hx, arrCoord[ez][ey][ex][icux_e[1]], arrCoord[ez][ey][ex][icux_e[2]], theta)*uxRef(arrCoord[ez][ey][ex][icux_e[0]] + hx, arrCoord[ez][ey][ex][icux_e[1]],
                                            arrCoord[ez][ey][ex][icux_e[2]], theta) - val_right)/hx;
...
static PetscErrorCode Derive_y(DM dmSol, Vec *pV2_y, Vec V2, PetscScalar theta)
{
    Vec             V2_y, coordLocal;
    PetscInt        startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscReal       hy;
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if(ey == 0) {
                        ...
                        row_l.loc = DOWN;
                        row_l.c = 0;
                        val_l = (val_left - uyRef(arrCoord[ez][ey][ex][icuy_e[0]], arrCoord[ez][ey][ex][icuy_e[1]] - hy, arrCoord[ez][ey][ex][icuy_e[2]], theta)*uyRef(arrCoord[ez][ey][ex][icuy_e[0]], arrCoord[ez][ey][ex][icuy_e[1]] - hy, arrCoord[ez][ey][ex][icuy_e[2]], theta))/hy;
                ...
                if(ey == N[1] - 1){
                        ...
                        row_r.loc = UP;
                        row_r.c = 0;
                        val_r = (uyRef(arrCoord[ez][ey][ex][icuy_e[0]], arrCoord[ez][ey][ex][icuy_e[1]] + hy, arrCoord[ez][ey][ex][icuy_e[2]], theta)*uyRef(arrCoord[ez][ey][ex][icuy_e[0]], arrCoord[ez][ey][ex][icuy_e[1]] + hy, arrCoord[ez][ey][ex][icuy_e[2]], theta) - val_right)/hy;
...
static PetscErrorCode Derive_z(DM dmSol, Vec *pW2_z, Vec W2, PetscScalar theta)
{
    Vec             W2_z, coordLocal;
    PetscInt        startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscReal       hz;
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if(ez == 0) {
                        ...
                        row_l.loc = BACK;
                        row_l.c = 0;
                        val_l = (val_left - uzRef(arrCoord[ez][ey][ex][icuz_e[0]], arrCoord[ez][ey][ex][icuz_e[1]], arrCoord[ez][ey][ex][icuz_e[2]] - hz, theta)*uzRef(arrCoord[ez][ey][ex][icuz_e[0]], arrCoord[ez][ey][ex][icuz_e[1]], arrCoord[ez][ey][ex][icuz_e[2]] - hz, theta))/hz;
                ...
                if(ez == N[2] - 1){
                        ...
                        row_r.loc = FRONT;
                        row_r.c = 0;
                        val_r = (uzRef(arrCoord[ez][ey][ex][icuz_e[0]], arrCoord[ez][ey][ex][icuz_e[1]], arrCoord[ez][ey][ex][icuz_e[2]] + hz, theta)*uzRef(arrCoord[ez][ey][ex][icuz_e[0]], arrCoord[ez][ey][ex][icuz_e[1]],
                                            arrCoord[ez][ey][ex][icuz_e[2]] + hz, theta) - val_right)/hz;
...
static PetscErrorCode ManageAdvection_x(PetscScalar dt, Vec* U_int, Vec U_n, Vec V_n, Vec W_n, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz, PetscScalar theta)
{
    ...
    DMCreateGlobalVector(dmSol_Staggered, &mixedFirst);
    {
        ...
        FirstShiftU_y(dmSol_Shifted, &U_y, U_shift, theta);
        FirstShiftU_z(dmSol_Shifted, &U_z, U_shift, theta);
        FirstShiftV_y(dmSol_Shifted, &V_y, V_shift, theta);
        FirstShiftW_z(dmSol_Shifted, &W_z, W_shift, theta); 
    ...
    CenterU(dmSol_Centered, &U_c, U_center, theta);

    Vec U2_x;
    DMCreateGlobalVector(dmSol_Centered, &U2_x);
    Derive_x(dmSol_Centered, &U2_x, U_c, theta);
...
static PetscErrorCode ManageAdvection_y(PetscScalar dt, Vec* V_int, Vec U_n, Vec V_n, Vec W_n, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz, PetscScalar theta)
{
...

    {
        ...
        FirstShiftV_y(dmSol_Shifted, &V_x, V_shift, theta);
        SecondShiftV_z(dmSol_Shifted, &V_z, V_shift, theta);

        FirstShiftU_y(dmSol_Shifted, &U_x, U_shift, theta);
        SecondShiftW_z(dmSol_Shifted, &W_z, W_shift, theta);
    ...
    CenterV(dmSol_Centered, &V_c, V_center, theta);

    Vec V2_y;
    DMCreateGlobalVector(dmSol_Centered, &V2_y);
    Derive_y(dmSol_Centered, &V2_y, V_c, theta);
...
static PetscErrorCode ManageAdvection_z(PetscScalar dt, Vec* W_int, Vec U_n, Vec V_n, Vec W_n, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz, PetscScalar theta)
{
    ...
    DMCreateGlobalVector(dmSol_Staggered, &mixedThird);
    {
        ...
        FirstShiftW_z(dmSol_Shifted, &W_x, W_shift, theta);// ==FirsShiftW_z
        SecondShiftW_z(dmSol_Shifted, &W_y, W_shift, theta);// ==SecondShiftW_z
        FirstShiftU_z(dmSol_Shifted, &U_x, U_shift, theta);// ==FirsShiftU_z
        SecondShiftV_z(dmSol_Shifted, &V_y, V_shift, theta);// ==SecondShiftV_z
    ...
    CenterW(dmSol_Centered, &W_c, W_center, theta);

    Vec W2_z;
    DMCreateGlobalVector(dmSol_Centered, &W2_z);
    Derive_z(dmSol_Centered, &W2_z, W_c, theta);
...
static PetscErrorCode Assemble_x(DM dmSol, Mat *pA, Vec *pRhs, Vec solRef, PetscScalar dt, PetscScalar Re, PetscScalar theta) {
            ...
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ex == 0) {
                    ...
                    valRhs = uxRef(arrCoord[ez][ey][ex][icux[0]], arrCoord[ez][ey][ex][icux[1]],
                                   arrCoord[ez][ey][ex][icux[2]], theta);
                    DMStagVecSetValuesStencil(dmSol, rhs, 1, &row, &valRhs, INSERT_VALUES);
                } else {
...
static PetscErrorCode Assemble_y(DM dmSol, Mat *pA, Vec *pRhs, Vec solRef, PetscScalar dt, PetscScalar Re, PetscScalar theta) {
...
static PetscErrorCode Assemble_z(DM dmSol, Mat *pA, Vec *pRhs, Vec solRef, PetscScalar dt, PetscScalar Re, PetscScalar theta) {
    Vec rhs, coordLocal;
    Mat A;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icuz[3], icuz_front[3];
...
static PetscErrorCode ManageViscosity(PetscScalar dt, PetscScalar Re, Vec* pU_pre, Vec* pV_pre, Vec* pW_pre, Vec U_int, Vec V_int, Vec W_int, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz, PetscScalar theta){
    ...
    CreateGrid(&dmSol_staggered, 0, 1, 0, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
    {
    Assemble_x(dmSol_staggered, &A_x, &rhs_x, U_int, dt, Re, theta);
    Vec U_pre;
    ...
    {
    Assemble_y(dmSol_staggered, &A_y, &rhs_y, V_int, dt, Re, theta);
    Vec V_pre;
    ...
    {
    Assemble_z(dmSol_staggered, &A_z, &rhs_z, W_int, dt, Re, theta);
    Vec W_pre;
...
static PetscErrorCode ComputeCell(DM dmSol, Vec *pDiv, Vec U_x, Vec U_y, Vec U_z) 
{
    ...
    for (ez = startz; ez < startz + nz; ++ez) { /* With DMStag, always iterate x fastest, y second fastest, z slowest */
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                PetscScalar val, val_U, val_V, val_W;
                ...
                DMStagVecGetValuesStencil(dmSol_y, l_y, 1, &row, &val_V);
                DMStagVecGetValuesStencil(dmSol, l_x, 1, &row, &val_U);
                DMStagVecGetValuesStencil(dmSol_z, l_z, 1, &row, &val_W);

                val = sqrt(val_U*val_U + val_V*val_V + val_W*val_W);

                DMStagVecSetValuesStencil(dmSol, div, 1, &row, &val, INSERT_VALUES);
...
static PetscErrorCode ManagePressure(PetscScalar dt, Vec* pP, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    DM dmSol_centered, dmSol_shifted, dmSol_staggered;
    Mat A;
    Vec rhs;
    ...
    Vec mean_int;
    DMCreateGlobalVector(dmSol_centered, &mean_int);
    VecSet(mean_int, sum);
    VecAXPY(rhs, -1.0, mean_int);
...
//////////////////////////////////////////////////
    Vec            nsp_vec;   // Null space vector
    MatNullSpace   nullsp;    // Null space context
    VecDuplicate(rhs, &nsp_vec); 
    VecSet(nsp_vec, 1.0);
    MatNullSpaceCreate(PETSC_COMM_WORLD, PETSC_TRUE, 1, &nsp_vec, &nullsp); 
...
//////////////////////////////////////////////////
    ...
    DMStagMigrateVec(dmSol_centered, *pP, dmSol_shifted, P_shifted);
            ...
            PetscViewer viewer_Pi;
            DM da_solution_P;
            DMStagCreateCompatibleDMStag(dmSol_shifted, 0, 0, 0, 1, &da_solution_P);
            Vec P_grid;
            DMStagVecSplitToDMDA(dmSol_shifted, P_shifted, ELEMENT, 0, &da_solution_P, &P_grid);
            PetscObjectSetName((PetscObject)P_grid, "p_simple_k");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "p_simple_k.vtr", FILE_MODE_WRITE, &viewer_Pi);
            VecView(P_grid, viewer_Pi);
            PetscViewerDestroy(&viewer_Pi); 
    ...
    PetscObjectDestroy((PetscObject*)&dmSol_staggered);
    VecDestroy(&nsp_vec);
    PetscObjectDestroy((PetscObject*)&nullsp);
    return 0;
}

static PetscErrorCode ManagePressure_x(Vec* pP_x, Vec P, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    DM dmSol_centered, dmSol_shifted, dmSol_staggered;
    ...
    DMStagMigrateVec(dmSol_centered, P, dmSol_shifted, P_shifted);
    ...
    Derive_x_P(dmSol_shifted, &P_x, P_shifted);
                ...
                /*PetscViewer viewer_Pi;
            DM da_solution_P;
            DMStagCreateCompatibleDMStag(dmSol_shifted, 0, 0, 1, 0, &da_solution_P);
            Vec P_grid;
            DMStagVecSplitToDMDA(dmSol_shifted, P_x, LEFT, 0, &da_solution_P, &P_grid);
            PetscObjectSetName((PetscObject)P_grid, "p_simple_k");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "p_simple_k.vtr", FILE_MODE_WRITE, &viewer_Pi);
            VecView(P_grid, viewer_Pi);
            PetscViewerDestroy(&viewer_Pi);*/
...
static PetscErrorCode ManagePressure_y(Vec* pP_y, Vec P, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    DM dmSol_centered, dmSol_shifted, dmSol_staggered;
...
static PetscErrorCode ManagePressure_z(Vec* pP_z, Vec P, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    DM dmSol_centered, dmSol_shifted, dmSol_staggered;
    ...
    Derive_z_P(dmSol_shifted, &P_z, P_shifted);
            ...
            DMStagCreateCompatibleDMStag(dmSol_shifted, 0, 0, 1, 0, &da_solution_P);
            Vec P_grid;
            DMStagVecSplitToDMDA(dmSol_shifted, P_z, BACK, 0, &da_solution_P, &P_grid);
            PetscObjectSetName((PetscObject)P_grid, "p_simple_k");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "p_simple_k.vtr", FILE_MODE_WRITE, &viewer_Pi);
            VecView(P_grid, viewer_Pi);
            PetscViewerDestroy(&viewer_Pi); */
...
static PetscErrorCode ComputeMagnitude(DM dmSol_Staggered_x, DM dmSol_Staggered_y, DM dmSol_Staggered_z, Vec* pMagnitude, Vec U, Vec V, Vec W, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz, PetscScalar theta){
    ...
    Vec U_interpolate;
    DMCreateGlobalVector(dmSol_Shifted, &U_interpolate);
    Vec U_shifted;
    DMCreateGlobalVector(dmSol_Shifted, &U_shifted);
    DMStagMigrateVec(dmSol_Staggered_x, U, dmSol_Shifted, U_shifted);

    Vec V_interpolate;
    DMCreateGlobalVector(dmSol_Shifted, &V_interpolate);
    Vec V_shifted;
    DMCreateGlobalVector(dmSol_Shifted, &V_shifted);
    DMStagMigrateVec(dmSol_Staggered_y, V, dmSol_Shifted, V_shifted);

    Vec W_interpolate;
    DMCreateGlobalVector(dmSol_Shifted, &W_interpolate);
    Vec W_shifted;
    DMCreateGlobalVector(dmSol_Shifted, &W_shifted);
    DMStagMigrateVec(dmSol_Staggered_z, W, dmSol_Shifted, W_shifted);

    CenterU_magnitude(dmSol_Shifted, &U_interpolate, U_shifted, theta);
    CenterV_magnitude(dmSol_Shifted, &V_interpolate, V_shifted, theta);
    CenterW_magnitude(dmSol_Shifted, &W_interpolate, W_shifted, theta);

    DM dmSol_Centered;
    CreateGrid(&dmSol_Centered, 0, 0, 1, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);

    Vec U_centered;
    DMCreateGlobalVector(dmSol_Centered, &U_centered);
    DMStagMigrateVec(dmSol_Shifted, U_interpolate, dmSol_Centered, U_centered);

    Vec V_centered;
    DMCreateGlobalVector(dmSol_Centered, &V_centered);
    DMStagMigrateVec(dmSol_Shifted, V_interpolate, dmSol_Centered, V_centered);

    Vec W_centered;
    DMCreateGlobalVector(dmSol_Centered, &W_centered);
    DMStagMigrateVec(dmSol_Shifted, W_interpolate, dmSol_Centered, W_centered);

    Vec Magnitude;
    DMCreateGlobalVector(dmSol_Centered, &Magnitude);



    ComputeCell(dmSol_Centered, &Magnitude, U_centered, V_centered, W_centered);

    VecCopy(Magnitude, *pMagnitude);

    VecDestroy(&Magnitude);

    VecDestroy(&U_centered);
    VecDestroy(&V_centered);
    VecDestroy(&W_centered);
    DMDestroy(&dmSol_Centered); 

    VecDestroy(&U_interpolate);
    VecDestroy(&V_interpolate);
    VecDestroy(&W_interpolate);

    VecDestroy(&U_shifted);
    VecDestroy(&V_shifted);
    VecDestroy(&W_shifted);   

    DMDestroy(&dmSol_Shifted);
...
int main(int argc, char **argv)
{
    ...
    static PetscReal Re   = 10.0;



    PetscScalar dt = 0.0001; //T / nt;
    ...
    Vec U_0, V_0, W_0;
    CreateReferenceSolutionFirst(dmSol_Staggered_x, &U_0, 0);
        PetscViewer viewer_Pi;
    DM da_solution_P;
    DMStagCreateCompatibleDMStag(dmSol_Staggered_x, 0, 0, 1, 0, &da_solution_P);
    Vec P_grid;
    DMStagVecSplitToDMDA(dmSol_Staggered_x, U_0, LEFT, 0, &da_solution_P, &P_grid);
    ...
    VecView(P_grid, viewer_Pi);
    PetscViewerDestroy(&viewer_Pi);
    CreateReferenceSolutionSecond(dmSol_Staggered_y, &V_0, 0);
    CreateReferenceSolutionThird(dmSol_Staggered_z, &W_0, 0);
    ...
    PetscScalar norm_inf_x;
    VecNorm(U_0, NORM_INFINITY, &norm_inf_x);
    std::cout << "Norm of U_0: " << norm_inf_x << std::endl;
    PetscScalar norm_inf_y;
    VecNorm(V_0, NORM_INFINITY, &norm_inf_y);
    std::cout << "Norm of V_0: " << norm_inf_y << std::endl;
    PetscScalar norm_inf_z;
    VecNorm(W_0, NORM_INFINITY, &norm_inf_z);
    std::cout << "Norm of W_0: " << norm_inf_z << std::endl;
    ...
    for(size_t i = 0; i < 10; i++){
        if (i == 0){
            ...
            /*Vec bench;
            DMCreateGlobalVector(dmSol_Staggered_x, &bench);
            CreateReferenceSolutionTry(dmSol_Staggered_x, &bench);
            PetscViewer viewer_Pi;
            DM da_solution_P;
            DMStagCreateCompatibleDMStag(dmSol_Staggered_x, 0, 0, 1, 0, &da_solution_P);
            Vec P_grid;
            DMStagVecSplitToDMDA(dmSol_Staggered_x, bench, LEFT, 0, &da_solution_P, &P_grid);
            PetscObjectSetName((PetscObject)P_grid, "bravo");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "bravo.vtr", FILE_MODE_WRITE, &viewer_Pi);
            VecView(P_grid, viewer_Pi);
            PetscViewerDestroy(&viewer_Pi);*/
            ...
            //theta = d*d*(i+1)*dt;
            theta = 3*(1/Re)*k*k*dt*(i+1);

            ManageViscosity(dt, Re, &U_pre, &V_pre, &W_pre, U_int, V_int, W_int, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta);

            //std::cout << "Spirit of Nebraska completed:   diffusion done." << std::endl;
            ManagePressure(dt, &P, U_pre, V_pre, W_pre, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
            ManagePressure_x(&P_x, P, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
            ManagePressure_y(&P_y, P, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
            ManagePressure_z(&P_z, P, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
            ...
            ComputeMagnitude(dmSol_Staggered_x, dmSol_Staggered_y, dmSol_Staggered_z, &Magnitude, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz, theta);

            PetscViewer viewer_magnitude;
            DM DM_magnitude;
            //DMStagCreateCompatibleDMStag(dmSol_Staggered_x, 0, 0, 1, 0, &DM_u);
            Vec magnitude;
            DMStagVecSplitToDMDA(dmSol_Centered, Magnitude, ELEMENT, 0, &DM_magnitude, &magnitude);
            ...
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)dmSol_Centered), filename_magnitude, FILE_MODE_WRITE, &viewer_magnitude);
            ...
            Vec bench;
            DMCreateGlobalVector(dmSol_Staggered_x, &bench);
            CreateReferenceSolutionTry(dmSol_Staggered_x, &bench, theta);
            PetscViewer viewer_Pi;
            DM da_solution_P;
            DMStagCreateCompatibleDMStag(dmSol_Staggered_x, 0, 0, 1, 0, &da_solution_P);
            Vec P_grid;
