...
{
    return -2 * pi * sin(4*pi*x)*sin(2*pi*y)*sin(2*pi*y)*sin(2*pi*z)*sin(2*pi*z);
...
static PetscErrorCode ManagePressure(PetscScalar dt, Vec* pP, Vec* pP_x, Vec*  pP_y, Vec* pP_z, Vec U_n, Vec V_n, Vec W_n, PetscScalar nx, PetscScalar ny, PetscScalar nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    Derive_x(dmSol_shifted, &P_x, P);
    Derive_y(dmSol_shifted, &P_y, P);
    Derive_z(dmSol_shifted, &P_z, P);
...
static PetscErrorCode DumpToVTK(DM dmSol_Staggered, Vec U, Vec V, Vec W) 
{
    
    PetscViewer viewer_U, viewer_V, viewer_W;
    ...
    //VecView(U_grid,PETSC_VIEWER_STDOUT_WORLD);

    //PetscPrintf(PETSC_COMM_WORLD,"Norm of x: %g\n",U_grid);

            //PetscViewerPushFormat(viewer_U, PETSC_VIEWER_ASCII_XML);
...
int main(int argc, char **argv)
{
    ...
    //DumpToVTK(dmSol_Staggered, U_pre, V_pre, W_pre);
    ...
    for(size_t i = 0; i <= nt; i++)
    {
        ...
        {   std::cout<<"ciao"<<std::endl;
            ...
            DMStagVecSplitToDMDA(dmSol_Staggered, U_int, LEFT, 0, &da_solution_P, &P_grid);
       
      
        
            PetscObjectSetName((PetscObject)P_grid, "cia");
     

            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "cia.vtr", FILE_MODE_WRITE, &viewer_P);
