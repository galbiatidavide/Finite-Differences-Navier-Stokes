...
static PetscScalar solution(PetscScalar x, PetscScalar y, PetscScalar z)
{
    ...
    return  -8*pi*cos(2*pi*x)*cos(2*pi*x)*cos(4*pi*y)*cos(8*pi*z)*cos(8*pi*z)*sin(4*pi*y) - 16*pi*cos(2*pi*x)*cos(2*pi*x)*cos(4*pi*y)*cos(4*pi*y)*cos(8*pi*z)*sin(8*pi*z);
}
...
static PetscErrorCode CenterU_magnitude(DM dmSol, Vec *pUCenter, Vec solRef)
{
...
static PetscErrorCode CenterV_magnitude(DM dmSol, Vec *pVCenter, Vec solRef) 
{
...
static PetscErrorCode CenterW_magnitude(DM dmSol, Vec *pWCenter, Vec solRef) 
{
...
static PetscErrorCode ManageAdvection_x(PetscScalar dt, Vec* U_int, Vec U_n, Vec V_n, Vec W_n, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
    ...
    Vec bench;
    DMCreateGlobalVector(dmSol_Staggered, &bench);
    CreateReferenceSolutionTry(dmSol_Staggered, &bench);
    CheckSolution(mixedFirst, bench);
            ...
            Vec P_grid;
            DMStagVecSplitToDMDA(dmSol_Staggered, mixedFirst, LEFT, 0, &da_solution_P, &P_grid);
...
static PetscErrorCode ComputeMagnitude(DM dmSol_Staggered_x, DM dmSol_Staggered_y, DM dmSol_Staggered_z, Vec* pMagnitude, Vec U, Vec V, Vec W, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz){
    ...
    CenterU_magnitude(dmSol_Shifted, &U_interpolate, U_shifted);
    CenterV_magnitude(dmSol_Shifted, &V_interpolate, V_shifted);
    CenterW_magnitude(dmSol_Shifted, &W_interpolate, W_shifted);
...
int main(int argc, char **argv)
{
    ...
    static PetscInt nx  = 120;
    static PetscInt ny  = 120;
    static PetscInt nz  = 120;
    ...
    for(size_t i = 0; i < 1; i++){
        if (i == 0){
            ...
            ComputeMagnitude(dmSol_Staggered_x, dmSol_Staggered_y, dmSol_Staggered_z, &Magnitude, U_up, V_up, W_up, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
