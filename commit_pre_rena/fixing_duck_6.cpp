...
int main(int argc, char **argv)
{
    ...
    static PetscReal Lx_0   = -1.0;
    static PetscReal Ly_0  = -1.0;
    static PetscReal Lz_0  = -1.0;
    ...
    CreateReferenceSolutionTry(dmSol_Staggered, &bench);
            ...
            Vec P_grid;
            DMStagVecSplitToDMDA(dmSol_Staggered, U_0, LEFT, 0, &da_solution_P, &P_grid);
            PetscObjectSetName((PetscObject)P_grid, "U0");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "U0.vtr", FILE_MODE_WRITE, &viewer_P);            
    ...
    for(size_t i = 0; i < 1; ++i){
        if(i == 0){
            ...
            PetscObjectSetName((PetscObject)P_grid, "new");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "new.vtr", FILE_MODE_WRITE, &viewer_P);            
            VecView(P_grid, viewer_P);
            PetscViewerDestroy(&viewer_P); 
            std::cout<<"ciao";
            if(rank == 0){
            ...
            PetscObjectSetName((PetscObject)P_grid, "ok");
            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "ok.vtr", FILE_MODE_WRITE, &viewer_P);            
            VecView(P_grid, viewer_P);
            PetscViewerDestroy(&viewer_P); 
            std::cout<<"ciao";
            }
