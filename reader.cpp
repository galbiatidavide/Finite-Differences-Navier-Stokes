...
int main(int argc, char* argv[]) {
    ...
    CreateGrid(&dm_stag, 0, 1, 0, nx_, ny_, nz_, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);

    Vec vec_stag, vec_cent;
    DMCreateGlobalVector(dm_cent, &vec_cent);
    DMCreateGlobalVector(dm_stag, &vec_stag);
    ...
    DMStagGetCorners(dm_stag, &startx, &starty, &startz, &nx, &ny, &nz, NULL, NULL, NULL);
    DMStagGetGlobalSizes(dm_stag, &N[0], &N[1], &N[2]);
    DMGetCoordinateDM(dm_stag, &dmCoord);

    DMGetCoordinates(dm_stag, &coord);
    ...
    DMStagGetLocationSlot(dm_stag, RIGHT, 0, &iux_right);
    DMStagGetLocationSlot(dm_stag, LEFT, 0, &iux_left);
    DMStagGetLocationSlot(dm_stag, UP, 0, &iux_up);
    DMStagGetLocationSlot(dm_stag, DOWN, 0, &iux_down);
    DMStagGetLocationSlot(dm_stag, BACK, 0, &iux_back);
    DMStagGetLocationSlot(dm_stag, FRONT, 0, &iux_front);

    DMCreateLocalVector(dm_stag, &vec_stag_local);
    DMGlobalToLocalBegin(dm_stag, vec_stag, INSERT_VALUES, vec_stag_local);
    DMGlobalToLocalEnd(dm_stag, vec_stag, INSERT_VALUES, vec_stag_local);
    DMStagVecGetArray(dm_stag, vec_stag_local, &arrVecStag);
    ...
    DMStagVecRestoreArray(dm_stag, vec_stag_local, &arrVecStag);
    DMLocalToGlobal(dm_stag, vec_stag_local, INSERT_VALUES, vec_stag);
    DMRestoreLocalVector(dm_stag, &vec_stag_local);

    PetscViewer viewer_stag;
    DM DM_stag;
    //DMStagCreateCompatibleDMStag(dmSol_Staggered_y, 0, 0, 1, 0, &DM_v);
    Vec rd;
    DMStagVecSplitToDMDA(dm_stag, vec_stag, DOWN, 0, &DM_stag, &rd);
    PetscObjectSetName((PetscObject)rd, "r_values_down");
    char filename_rd[50];
    sprintf(filename_rd, "results/r_values_down%03zu.vtr");
    PetscViewerVTKOpen(PetscObjectComm((PetscObject)dm_stag), filename_rd, FILE_MODE_WRITE, &viewer_stag);
