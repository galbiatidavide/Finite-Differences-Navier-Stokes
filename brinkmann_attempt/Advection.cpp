...
PetscErrorCode FirstShiftU_y(DM const & dmGrid, Vec & UShifted, Vec const & solRef, PetscScalar const & theta, Vec const & penalization) //ok
{
    ...
    DMGlobalToLocalBegin(dmGrid,penalization,INSERT_VALUES,penalizationLocal);
    DMGlobalToLocalEnd(dmGrid,penalization,INSERT_VALUES,penalizationLocal);
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ey == 0) {
                    DMStagStencil row_penalization;
                    PetscReal penalizationValue;
                    row_penalization.i = ex;
                    row_penalization.j = ey;
                    row_penalization.k = ez;
                    row_penalization.loc = DOWN;
                    row_penalization.c = 0;
                    DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization, &penalizationValue);
                    if(penalizationValue == 0.0) {
                        DMStagStencil row;
                        PetscScalar inter;
                        row.i = ex;
                        row.j = ey;
    ...
    PetscObjectDestroy((PetscObject*)&penalizationLocal);

    return 0;
}

PetscErrorCode FirstShiftU_z(DM const & dmGrid, Vec & UShifted, Vec const & solRef, PetscScalar const & theta, Vec const & penalization) //ok
{
...
PetscErrorCode FirstShiftV_y(DM const & dmGrid, Vec & VShifted, Vec const & solRef, PetscScalar const & theta, Vec const & penalization) //ok
{
...
PetscErrorCode FirstShiftW_z(DM const & dmGrid, Vec & WShifted, Vec const & solRef, PetscScalar const & theta, Vec const & penalization) //ok
{
...
PetscErrorCode SecondShiftV_z(DM const & dmGrid, Vec & VShifted, Vec const & solRef, PetscScalar const & theta, Vec const & penalization) //ok
{
...
PetscErrorCode SecondShiftW_z(DM const & dmGrid, Vec & WShifted, Vec const & solRef, PetscScalar const & theta, Vec const & penalization) //ok
{
...
PetscErrorCode CenterU(DM const & dmGrid, Vec & UCenter, Vec const & vec, PetscReal const & theta, Vec const & penalization)
{
    PetscInt icux_left[3], icux_right[3], iux_left, iux_right, iux_element;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    DM dmCoord;
    Vec vecLocal, vecULocal, coord, coordLocal, penalizationLocal;
    PetscReal ****arrCoord, ****arrVec, ****arrU, ****arrPenalization;
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ex == N[0] - 1) {
                    PetscReal penalization = arrPenalization[ez][ey][ex][iux_right];
                    if(penalization == 0.0) {
                        ...
                        arrU[ez][ey][ex][iux_element] = 0.0;
                    }
                } else if(ex == 0) {
                    PetscReal penalization = arrPenalization[ez][ey][ex][iux_left];
                    if(penalization == 0.0) {
                        PetscReal inter, next, prev;
                        next = uxRef(arrCoord[ez][ey][ex][icux_right[0]], arrCoord[ez][ey][ex][icux_right[1]], arrCoord[ez][ey][ex][icux_right[2]], theta);
                        prev = arrVec[ez][ey][ex][iux_left];
...
PetscErrorCode CenterV(DM const & dmGrid, Vec & VCenter, Vec const & vec, PetscReal const & theta, Vec const & penalization) 
{
    PetscInt icuy_down[3], icuy_up[3], iuy_up, iuy_down, iuy_element;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    DM dmCoord;
    Vec vecLocal, vecVLocal, coord, coordLocal, penalizationLocal;
    PetscReal ****arrCoord, ****arrVec, ****arrV, ****arrPenalization;   
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ey == N[1] - 1) {
                    PetscReal penalization = arrPenalization[ez][ey][ex][iuy_up];
                    if(penalization == 0.0) {
                        PetscReal inter, prev, next;
                        ...
                        arrV[ez][ey][ex][iuy_element] = 0.0;
                    }
                 } else if(ey == 0) {
                    PetscReal penalization = arrPenalization[ez][ey][ex][iuy_down];
                    if(penalization == 0.0) {
                        PetscReal inter, prev, next;
...
PetscErrorCode CenterW(DM const & dmGrid, Vec & WCenter, Vec const & vec, PetscReal const & theta, Vec const & penalization) 
{
    PetscInt icuz_back[3], icuz_front[3], iuz_back, iuz_front, iuz_element;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    DM dmCoord;
    Vec vecLocal, vecWLocal, coord, coordLocal, penalizationLocal;
    PetscReal ****arrCoord, ****arrVec, ****arrW, ****arrPenalization;   
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ez == N[2] - 1) {
                    PetscReal penalization = arrPenalization[ez][ey][ex][iuz_front];
                    if(penalization == 0.0) {
                        PetscReal inter, prev, next;
                        ...
                        arrW[ez][ey][ex][iuz_element] = 0.0;
                    }
                } else if(ez == 0) {
                    PetscReal penalization = arrPenalization[ez][ey][ex][iuz_back];
                    if(penalization == 0.0) {
                        PetscReal inter, prev, next;
...
PetscErrorCode Derive_x(DM const & dmGrid, Vec & U2_x, Vec const & vec, PetscReal const & theta, Vec const & penalization)
{
...
PetscErrorCode Derive_y(DM const & dmGrid, Vec & V2_y, Vec const & vec, PetscReal const & theta, Vec const & penalization)
{
...
PetscErrorCode Derive_z(DM const & dmGrid, Vec & W2_z, Vec const & vec, PetscReal const & theta, Vec const & penalization)
{
...
PetscErrorCode ManageAdvection_x(PetscReal const & dt, Vec & U_int, Vec const & U_n, Vec const & V_n, Vec const & W_n, PetscInt const & nx, PetscInt const & ny, PetscInt const & nz, PetscReal const & Lx_0, PetscReal const & Lx, PetscReal const & Ly_0, PetscReal const & Ly, PetscReal const & Lz_0, PetscReal const & Lz, PetscReal const & theta, Vec const & penalizationU, Vec const & penalizationV, Vec const & penalizationW)
{
...

    {
        ...
        FirstShiftU_y(dmGrid_Shifted, U_y, U_shift, theta, penalizationU);
        FirstShiftU_z(dmGrid_Shifted, U_z, U_shift, theta, penalizationU);
        FirstShiftV_y(dmGrid_Shifted, V_y, V_shift, theta, penalizationV);
        FirstShiftW_z(dmGrid_Shifted, W_z, W_shift, theta, penalizationW); 
    ...
    CenterU(dmGrid_Centered, U_c, U_center, theta, penalizationU);
    Vec U2_x;
    DMCreateGlobalVector(dmGrid_Centered, &U2_x);
    Derive_x(dmGrid_Centered, U2_x, U_c, theta, penalizationU);
    Vec homoFirst;
...
PetscErrorCode ManageAdvection_y(PetscReal const & dt, Vec & V_int, Vec const & U_n, Vec const & V_n, Vec const & W_n, PetscInt const & nx, PetscInt const & ny, PetscInt const & nz, PetscReal const & Lx_0, PetscReal const & Lx, PetscReal const & Ly_0, PetscReal const & Ly, PetscReal const & Lz_0, PetscReal const & Lz, PetscReal const & theta, Vec const & penalizationU, Vec const & penalizationV, Vec const & penalizationW)
{
    // Create necessary grids
    DM dmGrid_Shifted, dmGrid_Centered, dmGrid_Staggered;
    PetscFunctionBegin;
...

    {
        ...
        FirstShiftV_y(dmGrid_Shifted, V_x, V_shift, theta, penalizationV);
        SecondShiftV_z(dmGrid_Shifted, V_z, V_shift, theta, penalizationV);
        FirstShiftU_y(dmGrid_Shifted, U_x, U_shift, theta, penalizationU);
        SecondShiftW_z(dmGrid_Shifted, W_z, W_shift, theta, penalizationW);
    ...
    CenterV(dmGrid_Centered, V_c, V_center, theta, penalizationV);
    Vec V2_y;
    DMCreateGlobalVector(dmGrid_Centered, &V2_y);
    Derive_y(dmGrid_Centered, V2_y, V_c, theta, penalizationV);
    Vec homoSecond;
...
PetscErrorCode ManageAdvection_z(PetscReal const & dt, Vec & W_int, Vec const & U_n, Vec const & V_n, Vec const & W_n, PetscInt const & nx, PetscInt const & ny, PetscInt const & nz, PetscReal const & Lx_0, PetscReal const & Lx, PetscReal const & Ly_0, PetscReal const & Ly, PetscReal const & Lz_0, PetscReal const & Lz, PetscReal const & theta, Vec const & penalizationU, Vec const & penalizationV, Vec const & penalizationW)
{
    // Create necessary grids
    DM dmGrid_Shifted, dmGrid_Centered, dmGrid_Staggered;
    PetscFunctionBegin;
...

    {
        ...
        FirstShiftW_z(dmGrid_Shifted, W_x, W_shift, theta, penalizationW);// ==FirsShiftW_z
        SecondShiftW_z(dmGrid_Shifted, W_y, W_shift, theta, penalizationW);// ==SecondShiftW_z
        FirstShiftU_z(dmGrid_Shifted, U_x, U_shift, theta, penalizationU);// ==FirsShiftU_z
        SecondShiftV_z(dmGrid_Shifted, V_y, V_shift, theta, penalizationV);// ==SecondShiftV_
    ...
    CenterW(dmGrid_Centered, W_c, W_center, theta, penalizationW);
    Vec W2_z;
    DMCreateGlobalVector(dmGrid_Centered, &W2_z);
    Derive_z(dmGrid_Centered, W2_z, W_c, theta, penalizationW);
    Vec homoThird;
