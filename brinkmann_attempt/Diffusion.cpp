...
PetscErrorCode Assemble_x(DM const & dmGrid, Mat & A, Vec & rhs, Vec const & vec, Vec const & penalization, PetscReal const & dt, PetscReal const & Re, PetscReal const & theta) {
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                DMStagStencil row_penalization_left;
                PetscReal valPenalization_left;
                row_penalization_left.i = ex;
                row_penalization_left.j = ey;
                row_penalization_left.k = ez;
                row_penalization_left.loc =  LEFT;
                row_penalization_left.c = 0;
                DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization_left, &valPenalization_left);


                

                if (ex == N[0] - 1) {
                    DMStagStencil row_penalization_right;
                    PetscReal valPenalization_right;
                    row_penalization_right.i = ex;
                    row_penalization_right.j = ey;
                    row_penalization_right.k = ez;
                    row_penalization_right.loc =  RIGHT;
                    row_penalization_right.c = 0;
                    DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization_right, &valPenalization_right);

                    if(valPenalization_right == 0)
                    {                    
                        DMStagStencil row;
                        PetscReal valRhs;
                ...
                if (ex == 0) {
                    ...
                    if (ey == 0) {
                        if (ez == 0) {
                            ...
                            col[0].loc = LEFT;
                            col[0].c = 0;
                            valA[0] = -2.0 / (hx * hx) + -2.0 / (hy * hy) - 2.0 / (hz * hz) - Ret - Re*valPenalization_left;
                            col[1].i = ex;
                            col[1].j = ey + 1;
                            col[1].k = ez;
                            col[1].loc = LEFT;
...
PetscErrorCode Assemble_y(DM const & dmGrid, Mat & A, Vec & rhs, Vec const & vec, Vec const & penalization, PetscReal const & dt, PetscReal const & Re, PetscReal const & theta) {
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) { 
                ...
                DMStagStencil row_penalization_down;
                PetscReal valPenalization_down;
                row_penalization_down.i = ex;
                row_penalization_down.j = ey;
                row_penalization_down.k = ez;
                row_penalization_down.loc =  DOWN;
                row_penalization_down.c = 0;
                DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization_down, &valPenalization_down); 

                if (ey == N[1] - 1) {

                    DMStagStencil row_penalization_up;
                    PetscReal valPenalization_up;
                    row_penalization_up.i = ex;
                    row_penalization_up.j = ey;
                    row_penalization_up.k = ez;
                    row_penalization_up.loc =  UP;
                    row_penalization_up.c = 0;
                    DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization_up, &valPenalization_up);

                    if(valPenalization_up == 0)
                    {
                        /* Top velocity Dirichlet */
                        DMStagStencil row;
                        PetscReal valRhs;
                        const PetscReal valA = 1.0;
                        row.i = ex;
                ...
                if (ey == 0) {
                    ...
                    if (ex == 0) {
                        if (ez == 0) {
                            ...
                            col[0].loc = DOWN;
                            col[0].c   = 0;
                            valA[0]    = -2.0 / (hx * hx) + -2.0 / (hy * hy) - 2.0 / (hz * hz) - Ret - Re*valPenalization_down;
                            col[1].i   = ex;
                            col[1].j   = ey - 1;
                            col[1].k   = ez;
                            col[1].loc = DOWN;
...
PetscErrorCode Assemble_z(DM const & dmGrid, Mat & A, Vec & rhs, Vec const & vec, Vec const & penalization, PetscReal const & dt, PetscReal const & Re, PetscReal const & theta) {
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    ...
    for (ez = startz; ez < startz + nz; ++ez) {
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) { 
                ...
                DMStagStencil row_penalization_back;
                PetscReal valPenalization_back;
                row_penalization_back.i = ex;
                row_penalization_back.j = ey;
                row_penalization_back.k = ez;
                row_penalization_back.loc =  BACK;
                row_penalization_back.c = 0;
                DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization_back, &valPenalization_back);       

                if (ez == N[2] - 1) {
                    /* Front boundary velocity Dirichlet */
                    DMStagStencil row_penalization_front;
                    PetscReal valPenalization_front;
                    row_penalization_front.i = ex;
                    row_penalization_front.j = ey;
                    row_penalization_front.k = ez;
                    row_penalization_front.loc =  FRONT;
                    row_penalization_front.c = 0;
                    DMStagVecGetValuesStencil(dmGrid, penalizationLocal, 1, &row_penalization_front, &valPenalization_front);

                    if(valPenalization_front == 0.0) {
                        /* Front boundary velocity Dirichlet */
                        DMStagStencil     row;
                        PetscReal       valRhs;
                        const PetscReal valA = 1.0;
                ...
                if (ez == 0) {
                    ...
                    if (ex == 0) {
                        if (ey == 0) {
                            ...
                            col[0].loc = BACK;
                            col[0].c   = 0;
                            valA[0]    = -2.0 / (hx * hx) - 2.0 / (hy * hy) - 2.0 / (hz * hz) - Ret - Re*valPenalization_back;
                            col[1].i   = ex;
                            col[1].j   = ey + 1;
                            col[1].k   = ez;
                            col[1].loc = BACK;
...
PetscErrorCode ManageViscosity(DM const & dmGrid_x, DM const & dmGrid_y, DM const & dmGrid_z, PetscReal const & dt, PetscReal const & Re, Vec & U_pre, Vec & V_pre, Vec & W_pre, Vec const & U_int, Vec const & V_int, Vec const & W_int, Vec const & penalization_U, Vec const & penalization_V, Vec const & penalization_W, PetscReal const & theta){
    ...
    {
    Assemble_x(dmGrid_x, A_x, rhs_x, U_int, penalization_U, dt, Re, theta);
    ...
    {
    Assemble_y(dmGrid_y, A_y, rhs_y, V_int, penalization_V, dt, Re, theta);
    ...
    {
    Assemble_z(dmGrid_z, A_z, rhs_z, W_int, penalization_W, dt, Re, theta);
