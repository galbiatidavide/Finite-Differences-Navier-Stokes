...
PetscErrorCode UpdatebcU(DM const & dmGrid, Vec & U_up, PetscReal const & theta, Vec const & penalization)
{
    PetscInt icux_left[3], icux_right[3], iux_left, iux_right;
    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    DM dmCoord;
    Vec vecLocal, coord, coordLocal, penalizationLocal;
    PetscReal ****arrCoord, ****arrVec, ****arrPenalization;
...
PetscErrorCode UpdatebcV(DM const & dmGrid, Vec & V_up, PetscReal const & theta, Vec const & penalization) 
{
...
PetscErrorCode UpdatebcW(DM const & dmGrid, Vec & W_up, PetscReal const & theta, Vec const & penalization) 
{
...
PetscErrorCode UpdateVelocity(DM const & dmGrid_staggered_x, DM const & dmGrid_staggered_y, DM const & dmGrid_staggered_z, PetscReal const & dt, Vec & U_up, Vec & V_up, Vec & W_up, Vec const & P_x, Vec const & P_y, Vec const & P_z, Vec const & U_pre, Vec const & V_pre, Vec const & W_pre, PetscReal const & theta, Vec const & penalization_U, Vec const & penalization_V, Vec const & penalization_W)
{
    PetscFunctionBegin;
    ...
    UpdatebcU(dmGrid_staggered_x, U_up, theta, penalization_U);
    UpdatebcV(dmGrid_staggered_y, V_up, theta, penalization_V);
    UpdatebcW(dmGrid_staggered_z, W_up, theta, penalization_W);
