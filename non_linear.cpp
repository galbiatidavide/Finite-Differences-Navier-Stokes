...
static PetscScalar solution(PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar theta)
{
    ...
    //return sin((pi/3)*(x+y+z))*exp(-theta) + x*y*z;
    //return (A*sin(k*z) + C*cos(k*y))*exp(-theta);
    return cos(2*pi*x)*cos(4*pi*y)*cos(8*pi*z) + 0.01*8*pi*cos(2*pi*x)*cos(2*pi*x)*cos(4*pi*y)*cos(8*pi*z)*cos(8*pi*z)*sin(4*pi*y) + 0.01*16*pi*cos(2*pi*x)*cos(2*pi*x)*cos(4*pi*y)*cos(4*pi*y)*cos(8*pi*z)*sin(8*pi*z) + 0.01*4*pi*cos(2*pi*x)*cos(4*pi*y)*cos(4*pi*y)*cos(8*pi*z)*cos(8*pi*z)*sin(2*pi*x);
...
static PetscErrorCode CreateReferenceSolutionTry(DM dmSol, Vec *pSolRef, PetscScalar theta)
{
    ...
    for (ez = start[2]; ez < start[2] + n[2] + nExtra[2]; ++ez) {
        for (ey = start[1]; ey < start[1] + n[1] + nExtra[1]; ++ey) {
            for (ex = start[0]; ex < start[0] + n[0] + nExtra[0]; ++ex) {
                //if (ex < start[1] + n[1] && ey < start[2] + n[2]) 
                arrSol[ez][ey][ex][iuz] = solution(arrCoord[ez][ey][ex][icuz[0]], arrCoord[ez][ey][ex][icuz[1]], arrCoord[ez][ey][ex][icuz[2]], theta);
            }
        }
    }
    DMStagVecRestoreArrayRead(dmCoord, coordLocal, &arrCoord);
...
static PetscErrorCode FirstShiftU_y(DM const & dmSol, Vec & UShifted, Vec const & solRef, PetscScalar theta) //ok
{
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ey == 0) {
                    ...
                    PetscScalar next;
                    next = uxRef(arrCoord[ez][ey][ex][icux_left[0]], arrCoord[ez][ey][ex][icux_left[1]] - hy,
                ...
                if (ey == N[1] - 1) {
                    ...
                    PetscScalar next;
                    next = uxRef(arrCoord[ez][ey][ex][icux_right[0]], arrCoord[ez][ey][ex][icux_right[1]] + hy,
...
static PetscErrorCode FirstShiftU_z(DM const & dmSol, Vec & UShifted, Vec const & solRef, PetscScalar theta) //ok
{
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ez == 0) {
                    ...
                    PetscScalar next;
                    next = uxRef(arrCoord[ez][ey][ex][icux_left[0]], arrCoord[ez][ey][ex][icux_left[1]],
                ...
                if (ez == N[2] - 1) {
                    ...
                    PetscScalar next;
                    next = uxRef(arrCoord[ez][ey][ex][icux_right[0]], arrCoord[ez][ey][ex][icux_right[1]],
...
static PetscErrorCode FirstShiftV_y(DM const & dmSol, Vec & VShifted, Vec const & solRef, PetscScalar theta) //ok
{
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ex == 0) {
                    ...
                    PetscScalar next;
                    next = uyRef(arrCoord[ez][ey][ex][icuy_up[0]] - hx, arrCoord[ez][ey][ex][icuy_up[1]],
                ...
                if (ex == N[0] - 1) {
                    ...
                    PetscScalar next;
                    next = uyRef(arrCoord[ez][ey][ex][icuy[0]] + hx, arrCoord[ez][ey][ex][icuy[1]],
...
static PetscErrorCode FirstShiftW_z(DM const & dmSol, Vec & WShifted, Vec const & solRef, PetscScalar theta) //ok
{

    Vec coordLocal;

    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux_right[3], icux_back_left[3], icux_back_right[3], icux_front_left[3], icux_front_right[3], icuz[3], icuz_back[3];
    DM dmCoord;
    PetscScalar ****arrCoord;
    ...
    for (d = 0; d < 3; ++d) {
        ...
        DMStagGetLocationSlot(dmCoord, FRONT, d, &icuz[d]);
    }
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ex == 0) {
                    ...
                    PetscScalar next;
                    next = uzRef(arrCoord[ez][ey][ex][icuz[0]] - hx, arrCoord[ez][ey][ex][icuz[1]],
                                 arrCoord[ez][ey][ex][icuz[2]], theta);
                ...
                if (ex == N[0] - 1) {
                    ...
                    PetscScalar next;
                    next = uzRef(arrCoord[ez][ey][ex][icuz_back[0]] + hx, arrCoord[ez][ey][ex][icuz_back[1]],
                                 arrCoord[ez][ey][ex][icuz_back[2]], theta);
...
static PetscErrorCode SecondShiftV_z(DM const & dmSol, Vec & VShifted, Vec const & solRef, PetscScalar theta) //ok
{
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ez == N[2] - 1) {
                    ...
                    PetscScalar next;
                    next = uyRef(arrCoord[ez][ey][ex][icuy_up[0]], arrCoord[ez][ey][ex][icuy_up[1]],
                ...
                if (ez == 0) {
                    ...
                    PetscScalar next;
                    next = uyRef(arrCoord[ez][ey][ex][icuy[0]], arrCoord[ez][ey][ex][icuy[1]],
...
static PetscErrorCode SecondShiftW_z(DM const & dmSol, Vec & WShifted, Vec const & solRef, PetscScalar theta) //ok
{

    Vec coordLocal;

    PetscInt startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscInt icux_right[3], icux_back_up[3], icux_back_down[3], icux_front_up[3], icux_front_down[3], icuz_front[3], icuz_back[3];
    DM dmCoord;
    PetscScalar ****arrCoord;
        ...
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                if (ey == 0) {
                    ...
                    PetscScalar next;
                    next = uzRef(arrCoord[ez][ey][ex][icuz_front[0]], arrCoord[ez][ey][ex][icuz_front[1]] - hy,
                                 arrCoord[ez][ey][ex][icuz_front[2]], theta);
                ...
                if (ey == N[1] - 1) {
                    ...
                    PetscScalar next;
                    next = uzRef(arrCoord[ez][ey][ex][icuz_back[0]], arrCoord[ez][ey][ex][icuz_back[1]] + hy,
                                 arrCoord[ez][ey][ex][icuz_back[2]], theta);
...
static PetscErrorCode CenterU(DM const & dmSol, Vec & UCenter, Vec const & solRef, PetscScalar theta)
{
...
static PetscErrorCode CenterV(DM const & dmSol, Vec & VCenter, Vec const & solRef, PetscScalar theta) 
{
...
static PetscErrorCode CenterW(DM const & dmSol, Vec & WCenter, Vec const & solRef, PetscScalar theta) 
{
...
static PetscErrorCode Derive_x(DM const & dmSol, Vec & U2_x, Vec const & U2, PetscScalar theta)
{
    Vec             coordLocal;
    PetscInt        startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscReal       hx;
...
static PetscErrorCode Derive_y(DM const & dmSol, Vec & V2_y, Vec const & V2, PetscScalar theta)
{
    Vec             coordLocal;
    PetscInt        startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscReal       hy;
...
static PetscErrorCode Derive_z(DM const & dmSol, Vec & W2_z, Vec const & W2, PetscScalar theta)
{
    Vec             coordLocal;
    PetscInt        startx, starty, startz, N[3], nx, ny, nz, ex, ey, ez, d;
    PetscReal       hz;
...
int main(int argc, char **argv)
{
    ...
    Vec bench_x;
    DMCreateGlobalVector(dmSol_Staggered_z, &bench_x);
    CreateReferenceSolutionTry(dmSol_Staggered_z, &bench_x, 0);
