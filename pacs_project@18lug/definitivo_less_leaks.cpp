...
static PetscScalar solution(PetscScalar x, PetscScalar y, PetscScalar z)
{
    ...
    return  5*pi*pi*cos(pi*x)*sin(2*pi*y);
}
...
// Assembling advection term
static PetscErrorCode ManageAdvection_leaks(PetscScalar dt, Vec* U_int, Vec* V_int, Vec* W_int, Vec U_n, Vec V_n, Vec W_n, PetscInt nx, PetscInt ny, PetscInt nz, PetscScalar Lx_0, PetscScalar Lx, PetscScalar Ly_0, PetscScalar Ly, PetscScalar Lz_0, PetscScalar Lz)
{
...
int main(int argc, char **argv)
{
    ...
    for(size_t i = 0; i < 1; ++i){
        if(i == 0){
            ...
            ManageViscosity(dt, Re, &U_pre, &V_pre, &W_pre, bench, bench, bench, nx, ny, nz, Lx_0, Lx, Ly_0, Ly, Lz_0, Lz);
            ...
            DMStagVecSplitToDMDA(dmSol_Staggered, U_pre, LEFT, 0, &da_solution_P, &P_grid);



            PetscObjectSetName((PetscObject)P_grid, "pre");


            PetscViewerVTKOpen(PetscObjectComm((PetscObject)da_solution_P), "pre.vtr", FILE_MODE_WRITE, &viewer_P);
