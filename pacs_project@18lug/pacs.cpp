...

{
    ...
    //return  sin(2*pi*y)*sin(21*pi*x)*sin(2*pi*z);
}

static PetscScalar uyRef(PetscScalar x, PetscScalar y, PetscScalar z)
{
    return  -sin(pi*x)*cos(2*pi*y);    //return  sin(5*pi*y)*sin(27*pi*x)*sin(2*pi*z);
}
/*
    ...
    for (ez = startz; ez < startz + nz; ++ez) { /* With DMStag, always iterate x fastest, y second fastest, z slowest */
        for (ey = starty; ey < starty + ny; ++ey) {
            for (ex = startx; ex < startx + nx; ++ex) {
                ...
                row.loc = ELEMENT;
                row.c = 0;
                inter = uxRef(arrCoord[ez][ey][ex][icuy_e[0]], arrCoord[ez][ey][ex][icuy_e[1]],
                                 arrCoord[ez][ey][ex][icuy_e[2]]);
...
*/
   ...
   // {
        //if(i == 1)
        {
            ...
            DMStagVecSplitToDMDA(dmSol_Staggered, U_int, LEFT, 0, &da_solution_U, &U_grid);
            DMStagVecSplitToDMDA(dmSol_Staggered, V_int, DOWN, 0, &da_solution_V, &V_grid); 
            DMStagVecSplitToDMDA(dmSol_Staggered, W_int, BACK, 0, &da_solution_W, &W_grid); 
